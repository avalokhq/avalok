...
export default function StandaloneEnvServicesView({ envName, envDescription, onViewLogs, onBrowseFiles }: Props) {
  ...
  const { layout, changeLayout } = useLayoutToggle('avalok-sa-env-svc-layout')

  useEffect(() => {
    listStandaloneEnvServices(envName)
      .then(setServices)
  ...
  useEffect(() => {
    ...
    for (const svc of services) {
      checkStandaloneEnvService(envName, svc.name)
  ...
  const handleCheck = useCallback((svcName: string, e: React.MouseEvent) => {
    ...
    checkStandaloneEnvService(envName, svcName)
                ...
                render: (svc) => (
                  <div className="flex items-center justify-end gap-1">
                    ...
                    {svc.has_log_dir && onBrowseFiles && (
                      <IconButton onClick={(e) => { e.stopPropagation(); onBrowseFiles(svc) }} title="Browse files">

...
import { Box, ChevronRight, Boxes } from 'lucide-react'
  ...
  return (
    <div className="flex-1 overflow-auto">
      <div className="px-8 lg:px-16 py-8">
        ...
        {services.length === 0 ? (
          <EmptyState
            icon={<Box className="w-7 h-7 text-blue-400 opacity-60" />}
          ...
          <CollectionGrid>
            {services.map(svc => (
              <Card key={svc.name} hover padding="md" className="cursor-pointer" onClick={() => onSelectService(svc.name, svc.friendly_name || svc.name)}>
                <div className="w-8 h-8 rounded-lg flex items-center justify-center mb-3 bg-blue-500/10">
